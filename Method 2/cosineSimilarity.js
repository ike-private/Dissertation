...
posenet.load().then(function(net) {
  ...
  p0 = poses[0];
  console.log(p1);
  weightedDistance = pns.poseSimilarity(poses[0], poses[1], { strategy: 'weightedDistance' });
  poseData = [p1, p0];
...
let currentUserPose = p1;
let closestMatchIndex = findMostSimilarMatch(currentUserPose);
let closestMatch = poseData[closestMatchIndex];
console.log(closestMatch);
});

// $.getJSON("combo.json", function(json) {
//      poseData = json // this will show the info it in firebug console
// });

// poseData = JSON.parse($.getJSON({'url': "combo.json", 'async': false}).responseText);
// console.log(Object.entries(poseData.data));

function cosineDistanceMatching(poseVector1, poseVector2) {
    let cosineSimilarity = pns.poseSimilarity(poseVector1, poseVector2, { strategy: 'cosineDistance' });
